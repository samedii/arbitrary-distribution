...
class ToyProblem:
    ...
    def generate_target(self, data):
        ...
        target += torch.FloatTensor(data.size()).normal_(-10, 1)*indicator
        target += torch.FloatTensor(data.size()).normal_(10, 1)*(1-indicator)
        return target
...
def train(toy_problem, toy_prediction_model, batch_size=100, n_iterations=10000):
